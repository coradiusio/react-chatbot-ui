...
import Segment from './components/elements/Segment/Segment';
...
export default class FormBotApp extends React.Component {
  ...
  componentDidUpdate() {
    animateScroll(document.getElementById('react-chatbot-ui-messages-container'), 300);
  }
  ...
  submitInputValue(answerInput, formValue = '', source = 'text', fileName = '', fileExtension = '') {
    ...
    if (inputValidatedObject.success) {
      ...
      const inputElement = document.getElementById('react-chatbot-ui-input');
      if (inputElement) {
        inputElement.value = ''; // reset value as next will come
      }
  ...
  render() {
    ...
    return (
      <Segment id="bodyWrapper" className="boxWrapper raised padding-zero flexbox-parent border-none">
        <Header
          headerAs="h4"
          iconImagePath="/chat-bot-header-icon.png"
          iconName="user"
          iconColor="white"
          iconSize="big"
          ...
          headerSubtitleIcon="circle"
          headerSubtitleIconColor="green"
          headerSubtitleIconSize="tiny"
        />
        <Body
